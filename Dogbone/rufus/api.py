...
class InternalAPI:
    ...
    def __init__(self, env='LOCAL'):
        ...
        if self.ENVIRONMENT not in InternalAPI.ENVS:
            from exceptions import WrongEnvironmentException
            raise WrongEnvironmentException("env=%s is not a proper environment" % self.ENVIRONMENT)

    @property
    def API_ENDPOINT_USER(self):
        return self.ENVS[self.ENVIRONMENT]['PROTOCOL'] + '://' + self.ENVS[self.ENVIRONMENT]['DOMAIN'] + '/api/v1/_internal/user/%s'

    @property
    def API_ENDPOINT_USER_CREATE(self):
        return self.ENVS[self.ENVIRONMENT]['PROTOCOL'] + '://' + self.ENVS[self.ENVIRONMENT]['DOMAIN'] + '/api/v1/_internal/user?send_auto_create=true'

    @property
    def API_ENDPOINT_DOCUMENT_UPLOAD(self):
        return self.ENVS[self.ENVIRONMENT]['PROTOCOL'] + '://' + self.ENVS[self.ENVIRONMENT]['DOMAIN'] + '/api/v1/_internal/document/upload?from=%s&send_upload_via_email=true'

    @property
    def API_ENDPOINT_SUBSCRIPTION_CREATE(self):
        return self.ENVS[self.ENVIRONMENT]['PROTOCOL'] + '://' + self.ENVS[self.ENVIRONMENT]['DOMAIN'] + '/api/v1/_internal/user/%s/subscription/%s'

    @property
    def API_SEND_UNSUPPORTED_FILE_TYPE_NOTIFICATION(self):
        return self.ENVS[self.ENVIRONMENT]['PROTOCOL'] + '://' + self.ENVS[self.ENVIRONMENT]['DOMAIN'] + '/api/v1/_internal/user/%s/notify/unsupported_file_type'

    @property
    def API_SEND_HELP_NOTIFICATION(self):
        return self.ENVS[self.ENVIRONMENT]['PROTOCOL'] + '://' + self.ENVS[self.ENVIRONMENT]['DOMAIN'] + '/api/v1/_internal/user/%s/notify/help'

    @property
    def API_SEND_ATTACHMENTS_NOT_FOUND_NOTIFICATION(self):
        return self.ENVS[self.ENVIRONMENT]['PROTOCOL'] + '://' + self.ENVS[self.ENVIRONMENT]['DOMAIN'] + '/api/v1/_internal/user/%s/notify/attachments_not_found'
