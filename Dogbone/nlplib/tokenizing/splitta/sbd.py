...
class Model:
    ...
    def load(self):
        ...
        """
        self.feats = sbd_util.load_pickle(os.path.join(self.path, 'feats'))
        self.lower_words = sbd_util.load_pickle(os.path.join(self.path, 'lower_words'))
        self.non_abbrs = sbd_util.load_pickle(os.path.join(self.path, 'non_abbrs'))
...
def load_sbd_model(model_path='model_nb/', use_svm=False, verbose=False):
    if verbose:
