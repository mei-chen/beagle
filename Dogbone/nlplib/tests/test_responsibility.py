...
class ResponsibilityTest(TestCase):
    ...
    def test_dummy(self):
        ...
        for idx, (noli, them_party, you_party) in enumerate(resp):
            ...
            actual_lens = (
                len(filter(filter_fn, facade.responsibilities(them_party).iteritems())),
                len(filter(filter_fn, facade.responsibilities(you_party).iteritems())),
                len(filter(filter_fn, facade.responsibilities('both').iteritems())),
            )
