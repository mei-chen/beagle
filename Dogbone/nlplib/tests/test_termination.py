...
class TerminationTest(TestCase):
    ...
    def test_dummy(self):
        ...
        for idx, (noli, them_party, you_party) in enumerate(term):
            ...
            actual_lens = (
                len(list(filter(filter_fn, facade.terminations(them_party).items()))),
                len(list(filter(filter_fn, facade.terminations(you_party).items()))),
                len(list(filter(filter_fn, facade.terminations('both').items()))),
            )
