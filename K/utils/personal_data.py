...
DOLLAR_TYPES = ['australian', 'bahamian', 'barbados', 'belize', 'bermudian', 'brunei', 'canadian', 'cayman islands', 'east caribbean', 'fiji', 'guyana', 'hong kong', 'jamaican', 'liberian', 'namibia', 'new taiwan', 'new zealand', 'singapore', 'solomon islands', 'surinam', 'trinidad and tobago', 'us', 'zimbabwe']
...
REGEX_PATTERNS = {
    ...
    'Price': (
        ...
        re.compile(
            ur'(?i)(?:[$¥£€₽₴¢₡₪₩₮฿₱؋]\s*)?[+-]?[0-9]{1,12}(?:,?[0-9]{3})*(?:\.[0-9]{1,6})?\s+(?:' + '|'.join(DOLLAR_TYPES) + ')\s+dollars?'
        ),
